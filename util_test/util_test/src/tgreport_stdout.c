...
static INLINE void reportBegin(TGReporter *self,const char *groupName, int numTests)
{
...
static INLINE void reportTest(TGReporter *self,const char *testName)
{
...
static INLINE void reportPass(TGReporter *self)
{
	printf(" [OK]\n");
	(void)self;
}

static INLINE const char *  operatorString(TAOperator op)
{
    switch (op)
    {
    case TAO_EQ:
...
static INLINE void reportPosition(const TestAssertion *ta)
{
    printf("    In function: %s\n    In file: %s\n    In line: %d\n",
            TA_getFunc(ta),TA_getFile(ta),TA_getLine(ta));
}
static INLINE void reportFailBase(const TestAssertion *ta)
{
    printf(" [FAIL]\n    Failure:%s\n    Reason:%s\n",TA_getMessage(ta),TA_getReason(ta));
}
static INLINE void reportFail_bool(const TestAssertion *ta)
{
...
static INLINE void reportFail_num(const TestAssertion *ta)
{
...
static INLINE void reportFail_mem(const TestAssertion *ta)
{
...
static INLINE void reportFailedAssert(const TestAssertion *ta)
{
...
static INLINE void reportFail(TGReporter *self,const TestAssertion *ta)
{
    reportFailBase(ta);
...
static INLINE void reportErrorBase(const TestAssertion *ta)
{
    printf(" [ERROR]\n    Error:%s\n    Reason:%s\n",TA_getMessage(ta),TA_getReason(ta));
}

static INLINE void reportError(TGReporter *self,const TestAssertion *ta)
{
    reportErrorBase(ta);
    reportFailedAssert(ta);
    reportPosition(ta);
...
static INLINE void reportSummary(TGReporter *self,const TestGroupOutcome *summary)
{
